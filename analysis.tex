...
This mistiming of L1 trigger signals led to a significant contribution of events
with high electromagnetic activity in the ECAL encaps to trigger an event
acceptance incorrectly associated with an proton bunch crossing.
This effect, known as pre-firing, is significant because requirements necessary
to avoid a excessive readout of events prevent events from successive bunch crossings
from triggering. That is, if an event has been accepted by the CMS trigger
system, it is impossible for the following event to also be accepted.

For EW WZ events, forward and high momentum jets can lead to such L1 pre-firing.
...
It is therefore likely to be rejected by the HLT, leading to an unrecoverable 
inefficiency.
...
signal from firing the trigger, but do not prevent the correctly timed event
from triggering. Jet candidates in the reconstructed data are formed 
using the anti-$\kt$ algorithm and required to have $\pt > 30\GeV$
and are matched to their trigger primitive objects.
The jet pre-firing efficiency is defined as the ratio of jet objects matched
to the correct bunch crossing to the total jet  

\begin{figure}[htbp]
  \centering
   \includegraphics[width=0.4\textwidth]{figures/AnalysisProcedure/jet0Eta_prefiring.pdf}
   \includegraphics[width=0.4\textwidth]{figures/AnalysisProcedure/jet1Eta_prefiring.pdf}
   \includegraphics[width=0.4\textwidth]{figures/AnalysisProcedure/mjj_prefiring.pdf}
   \includegraphics[width=0.4\textwidth]{figures/AnalysisProcedure/dEtajj_prefiring.pdf}
  \caption{
...
in simulated events for \EWWZ process selected for the analysis.
...
described in Section~\ref{sec:reconstruction}.
...
formulae for the , presented in Ref.~\ref{Cowan:2010js}.
