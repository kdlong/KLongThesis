...
fermions---the quarks and leptons, the spin 0 Higgs field, and the gauge fields:
\begin{equation}
...
fermionic fields.
A gauge transformations is ``generated''
by the Lie group defined by a set of $n\times n$ matrices $t^{a}$.
Infinitesimal unitary rotations of the fermion fields of the form $\exp{iV(x)}$,
for an arbitrary $n\times n$ matrix $V(x)$, can be expressed
...
the expression is naturally invariant under these transformations.
...
a field $B^{\mu}$, which itself transforms according to the 
...
to the isovector field $\mathbf{B^{\mu}}$ and isoscalar $A_{\mu}$ respectively. 
...
theory. This cannot be a correct theory of nature. In particular, we have already
emphasized the critical relationship between the mass of the $\Wpm$ and $\PZ$ bosons 
and the weakness of the interaction. 

\section{Electroweak symmetry breaking}

The solution to this seemingly insurmountable issue 
comes from the $\mathcal{L}_{\text{scalar}}$ term of Equation~\ref{eq:smlagrangian}
through the phenomena of EWSB.
The EWSB is which the underlying symmetry of a fundamental theory is hidden from its
observable properties. 
Consider an isospin doublet scalar field $\phi$ with self interactions defined by the Lagrangian
\begin{equation}
...
distributions of selected \WZ events with a VBS topology. 
